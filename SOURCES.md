# Sources & References (OCP 4.18--4.22; RHEL 9 where applicable)
...
> **Multi-version links:** Where a URL contains `/4.18/`, replace with the target version (e.g., `/4.20/`) for version-specific content.
...
- **oc-mirror v1 deprecation notice** — OCP 4.18+\
  *oc-mirror v1 (apiVersion: mirror.openshift.io/v1alpha2) is deprecated. v2 (v2alpha1) is the default and supported path. v1 imagesets in this repo's 4.18/v1/ directory are kept for legacy reference only.*
...
- **AWS VPC subnets with roles** — OCP 4.19+\
  *Structured `platform.aws.vpc.subnets` with role-based subnet assignment. See 4.19+ install-config parameter docs for the updated schema.*
...
- **ClusterUserDefinedNetwork (CUDN)** — OCP 4.19+\
  *Declarative localnet topology management. Auto-creates NADs in selected namespaces. See 4.19+ networking docs for the ClusterUserDefinedNetwork API.*
...
- **OVN-Kubernetes**\
  ...
  *OVN localnet topology, bridge-mappings, and secondary network support.*
