...
**Multi-VLAN options (alternatives to the OVN localnet trunk shown here)**

1) **Per-VLAN localnet NADs:** Create separate OVN localnet NADs with `"vlan": <id>` to isolate traffic at the NAD level rather than in-guest.
2) **cnv-bridge per-VLAN NADs:** Use a Linux bridge NNCP instead of OVS, and create per-VLAN NADs with `"type": "cnv-bridge"` and `"vlan": <id>`.

Choose one approach per cluster and keep it consistent across VMs.
